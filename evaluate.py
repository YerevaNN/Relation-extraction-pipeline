...
"""
Original evaluation script for Biomedical Relation eXtraction Task (BRXT).
...
def main():
    ...
    #             h = hash_sentence(t, args.match_by)
    #             for path in prediction:
    #                 if hash_sentence(path, args.match_by) == h:
    #                     filtered_prediction.append(path)
    #
    ...
    #     prediction = [path for path in prediction if tags(path, args.tags)]
    #
    # if args.has_sdg != 0:
    #     raise Exception('Not implemented in `union` mode')
    #     if args.has_sdg == 1:
    ...
    #     for path in prediction:
    #         path['extracted_information'] = [x for x in path['extracted_information'] if has_sdg_filter(x)]
